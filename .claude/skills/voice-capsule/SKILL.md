...
name: voice-capsule
version: "0.1.0"
description: 원문과 한국어 문체 프리셋을 받아, 프리셋 톤에 가깝게 원문을 다시 쓰되 사실·수치·고유명사·직접인용은 토큰 단위로 보존한다. 프리셋 인자가 명시될 때 발동된다.
---
...
- `capsule_path`: `voices/{default|user}/{voice-id}/` 디렉터리 (= 문체 프리셋). `fingerprint.json` + `descriptor.md` 필수, `exemplars.md` 권장.
...
자연어 트리거 키워드 사전 없음. 프리셋 인자가 명시되면 발동한다.

- 슬래시 커맨드: `/voice-capsule <원문> --capsule <프리셋경로>`
- 자연어: "이 글, `voices/default/concrete-essay-ko` 프리셋으로 윤문해줘"

## 한계

- 프리셋이 침묵하는 차원은 원문 보존 (프리셋에 "유머 톤" 묘사 없으면 유머는 안 살린다).
- 자동 검수 루프 없음. invariant 위반 edit 1건은 self-rollback, 그 이상은 사람 호출.
- 5,000자 초과는 분할 처리 권장.

## 참고

- 프리셋 형식: [`references/capsule-schema.md`](references/capsule-schema.md)
