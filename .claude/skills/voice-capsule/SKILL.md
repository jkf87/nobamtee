...
`_workspace/{YYYY-MM-DD-NNN}/final.md` — 윤문본. 본문 끝 `<!-- VC-SUMMARY -->` 주석 블록 1개에 메트릭·diff·플래그를 동봉.

`_workspace/` 에 raw 입력 복사본은 두지 않는다.

## 절차 — 단일 에이전트 3콜 캡

1. **Read 캡슐** (`descriptor.md` + `fingerprint.json` + 있으면 `exemplars.md` 를 한 번에).
2. **Read 원문**.
3. 메모리 안에서:
   - 원문 메트릭 ← 문장 길이 분포, 종결어미, `~할 수 있다·~되어진다·~에 의해`, 접속사 밀도, 영어 외래어 비율 등 캡슐 fingerprint 와 동일 축.
   - 거리 d(원문, 캡슐) = 축별 정규 차이의 가중합.
   - 거리를 크게 만드는 구간(문장·구문) 이 윤문 후보. 후보 외 구간은 건드리지 않는다.
   - Do-NOT 토큰(수치·고유명사·인용·영어 약어·`donot_keywords`)은 보존. 후보 안에 끼어 있어도 그 토큰만 보호하고 주변만 다시 쓴다.
   - 재작성은 descriptor 톤 묘사를 따르고, exemplars 가 있으면 그 리듬을 참조한다. 캡슐이 침묵하는 차원은 원문 그대로.
   - 변경률 30% 초과 → 경고 플래그, 50% 초과 → 안전 버전(≤30%)으로 롤백.
4. **Write `final.md`** (총 3 콜).
...
- 캡슐이 침묵하는 차원은 원문 보존 (예: 캡슐에 "유머 톤" 묘사 없으면 유머는 안 살린다).
- 자동 검수 루프 없음. invariant 위반 edit 1건 self-rollback 1회. 그 이상은 사람 호출.
- 5,000자 초과는 분할 처리 권장.
