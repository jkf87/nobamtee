...
3. 거리를 키우는 표지를 가진 문장·구문이 후보:
   - 어휘·문법 표지: `~할 수 있다`, `~되어진다`, `~에 의해`, 추상부사 (`결과적으로`, `따라서`, `즉`), 영어 직역 어휘 (`존재한다`, `가지고 있다`), 이중 피동, 명사 앞 ≥3어절 관형구.
   - 구조 표지 (fingerprint 의 `forbidden_structural_patterns` 가 정의됐다면): 표·코드·리스트 직후 마무리 메타코멘트, `X 만 하면 Y` 평행 단언, `A 가 아니라 B 다` 평행 부정, em-dash 부연 연쇄, 자기과시 메타, 리스트 직후 재요약.
   - 보조 검출기 (fingerprint 의 `auxiliary_detectors` 가 정의됐다면): im-not-ai taxonomy v2.0 에서 흡수한 S1·S2 패턴도 거리를 키우는 후보로 함께 검출. 각 항목의 `regex` 로 1차 매칭, `category` 별 카운트 임계 (D-1·I-4 류) 가 명시되어 있으면 그 임계를 적용.
