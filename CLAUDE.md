...
한국어 "AI 밤티" 제거기. 보이스 캡슐 한 축으로 동작.

## 원리

원문 메트릭과 캡슐 fingerprint 의 거리를 좁히는 방향으로, 거리를 키우는 구간만 다시 쓴다. 사실·수치·고유명사·인용은 토큰 단위로 보존. 패턴 사전·카테고리·트리거 키워드 없음.
...
- 캡슐 디렉터리는 `fingerprint.json` · `descriptor.md` · `PROVENANCE.md` 필수.
- 캡슐 정의는 [`SKILL.md`](.claude/skills/voice-capsule/SKILL.md) 참조.

## 한계

- 보이스 변환 깊이는 캡슐 descriptor + exemplars 의 풍부함을 따른다.
