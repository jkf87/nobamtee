...
사람이 자연스럽게 쓴 글을 분석하면 한국어 글의 "모양" 이 수치로 남습니다. 평균 문장 길이, 종결어미 `~다` 의 비율, `~할 수 있다` 가 1,000자에 몇 번 나오는지, 접속사 밀도, 영어 외래어 비율 — 표면 통계 10여 개입니다.

AI가 쓴 글은 이 통계에서 사람 글과 다른 자리를 차지합니다. `~할 수 있다` 가 잦아지고, 문장 길이가 균일해지고, 접속사가 단락마다 박힙니다.

윤문은 이 거리를 좁히는 작업입니다. 지표별로 원문과 프리셋 사이의 차이를 측정해 합치면 한 숫자로 거리가 나옵니다. 거리를 키우는 구간만 — `~할 수 있다` 가 박힌 문장, 같은 종결어미가 반복되는 문단 — 다시 씁니다. 사실·수치·고유명사·인용은 토큰 단위로 그대로 둡니다.
