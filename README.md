...
한국어 글의 "AI 밤티" — 통계적으로 튀는 저품질 특유 표식 — 를 패턴 사전 없이 **보이스 캡슐 한 개**로 제거한다. 엔진은 `voice-capsule`, 산출 컨셉은 "노밤티".

---

## 컨셉

AI가 쓴 한국어 글의 티는 카테고리 40+개를 외워야 잡히는 게 아니다. **"사람이 쓴 글"의 좌표와 얼마나 멀리 있느냐**의 거리로 환산된다. 그 거리를 한국어 문장 표면의 정량 지표 — 문장 길이 분포, 종결어미 비율, `~할 수 있다` 빈도, 접속사 밀도, 영어 외래어 비율, post-editese 3축 등 — 으로 측정하면 "AI 같은가" 는 한 줄 수치가 된다.

같은 원리로 윤문도 한다. 원문 메트릭과 캡슐 fingerprint 의 거리를 좁히는 방향으로, **거리를 크게 만드는 구간만** 다시 쓴다. 나머지 — 사실·수치·고유명사·인용 — 은 한 글자도 건드리지 않는다.

보이스 캡슐은 산문 그 자체가 아니다. **세 겹의 파생 표현**이다.

| 레이어 | 정체 | 분량 |
|---|---|---|
| A. `fingerprint.json` | 보이스의 정량 지표 | ~10–15개 수치 |
| B. `descriptor.md` | 톤 묘사. 사람이 읽고 보이스를 머릿속에 그릴 수 있는 산문 | ≤300자 |
...
윤문본은 다음을 **token level** 로 보존한다. 위반 시 그 edit 만 롤백.
...
장르가 맞지 않는 캡슐을 쓰면 결과물에 "장르 미스매치 AI 티"가 새로 생긴다 — 기술 글이 문학적으로, 문학이 기술적으로. 캡슐은 register 와 맞춰 고른다.
...
산출물은 `_workspace/{YYYY-MM-DD-NNN}/final.md`. 본문 끝 `<!-- VC-SUMMARY -->` HTML 주석 블록에 메트릭·diff·플래그가 동봉되어 마크다운 뷰어에는 본문만 노출된다.
